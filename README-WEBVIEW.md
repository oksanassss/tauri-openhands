...
### Custom (Rust Backend Integration)

**Component name:** `custom`

**Events:**

- `action`: Send a custom action to the Rust backend
  - Parameters:
...
- `received`: Sent immediately when the message is received by the native bridge
  - Data:
    - `status`: Status of the message reception
    - `message`: Acknowledgment message
    - `timestamp`: Timestamp of when the message was received

- `rust-result`: Sent when the Rust backend processes the message
  - Data:
    - `processedBy`: Identifier of the processor
    - `rustTimestamp`: Timestamp from the Rust backend
    - `originalTimestamp`: Original timestamp from the request
    - `message`: Response message
    - `randomValue`: A random value generated by the Rust backend
...
4. Connecting to the Tauri Rust backend for full round-trip communication

### Rust Backend Integration

The bridge includes integration with the Tauri Rust backend, allowing messages to be forwarded from web content to Rust and from Rust back to web content.

#### Rust Commands

The following Rust commands are available:

- `handle_strada_message`: Handles messages from the bridge and returns a response
- `send_strada_message`: Sends a message from Rust to the web content
...
#### Example: Custom Component with Rust Backend

```javascript
// Register a component
const custom = Strada.registerComponent("custom");
...
document.addEventListener("strada:custom:rust-result", function(event) {
    console.log("Rust result:", event.detail);
});
```

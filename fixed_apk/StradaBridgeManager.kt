...
class StradaBridgeManager(private val context: Context) {
    ...
    /**
     * Initialize the bridge by injecting JavaScript and setting up the JavascriptInterface.
     * This is a public method that can be called explicitly to ensure the bridge is initialized.
     */
    fun initialize() {
        initializeComponents()
        
        // First, add the JavaScript interface
        webView?.addJavascriptInterface(this, "StradaNativeBridge")
        Log.d(TAG, "Added JavascriptInterface as StradaNativeBridge")
        
        // Then post the JavaScript initialization to ensure it runs on the UI thread
        webView?.post {
            val js = """
                if (typeof window.StradaNativeBridge === 'undefined' || !window.StradaNativeBridge) {
                    console.log('Initializing StradaNativeBridge from native code');
                    ...
                    // Initialize StradaBridge if it exists
                    if (window.StradaBridge && typeof window.StradaBridge.init === 'function') {
                        console.log('Calling StradaBridge.init() from native code');
                        window.StradaBridge.init();
                    } else {
                        console.warn('StradaBridge not found or init method not available');
                    }
                    
                    // Dispatch the ready event
                    console.log('Dispatching strada-native-ready event');
                    document.dispatchEvent(new CustomEvent('strada-native-ready'));
                } else {
                    console.log('StradaNativeBridge already initialized');
                }
            """.trimIndent()
            
            webView?.evaluateJavascript(js) { result ->
                Log.d(TAG, "Bridge initialization script executed with result: $result")
                
                // After a short delay, check if the bridge is working
                webView?.postDelayed({
                    webView?.evaluateJavascript(
                        "console.log('Bridge check: StradaNativeBridge exists:', !!window.StradaNativeBridge); " +
                        "!!window.StradaNativeBridge;", 
                    ) { checkResult ->
                        Log.d(TAG, "Bridge check result: $checkResult")
                    }
                }, 500)
            }
        }
